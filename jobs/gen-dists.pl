...
while $sth.fetchrow_hashref -> $m {
    ...
    %dist-lines{$dist-letter} ~= dist-line($m, %dist-quality{$m<distauth>}{$m<distname>},
        "dist/&uri_encode($dist-letter)/&encode_punycode($m<distname>)/&encode_punycode($m<distauth>).html");
}
...
while $sth.fetchrow_hashref -> $m {
    ...
    %auth-lines{$auth-letter} ~= dist-line($m, %dist-quality{$m<distauth>}{$m<distname>},
        "auth/&uri_encode($auth-letter)/&encode_punycode($m<distauth>)/&encode_punycode($m<distname>).html");
}
